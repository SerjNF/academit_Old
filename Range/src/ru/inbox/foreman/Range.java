...
public class Range {
    ...
    static ArrayList<Range> sumOfRanges(Range rangeOne, Range rangeTwo) {
        ...
        if (rangeOne.isInside(rangeTwoFrom) || rangeOne.isInside(rangeTwoTo) || rangeTwo.isInside(rangeOneFrom) || rangeTwo.isInside(rangeOneTo)) {
     ...
     * @return ArrayList  диапазонов. Результат может состоять из однго или дву диапазонов. Если вычитаем равные диапазоны или из меньшего больший, возвращает ноль
     */
    static ArrayList<Range> subtractOfRange(Range rangeOne, Range rangeTwo) {
        ...
        if (rangeOne.isInside(rangeTwoFrom) && rangeOne.isInside(rangeTwoTo)) {
            ...
            if (rangeOneFrom != rangeTwoFrom) {
                resultRange.add(new Range(rangeOneFrom, rangeTwoFrom));
            }
            if (rangeTwoTo != rangeOneTo) {
